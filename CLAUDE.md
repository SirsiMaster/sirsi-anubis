...
*   **Why**: Without a live loop, items addressed to a registered thread sit unread until a human types `ctr`. Codex already had this via its heartbeat worker; this rule gives every Claude thread the same parity so the multi-agent relay (A26) actually completes without manual nudging.
