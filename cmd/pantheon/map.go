...
import (
	...
	"github.com/SirsiMaster/sirsi-pantheon/internal/mapper"
...
force-directed graph. Hover nodes for details, scroll to zoom, drag to pan.
  ...
  anubis seba                    Generate map and open in browser
  anubis seba --output map.html  Save to a specific file
  anubis seba --json             Export graph data as JSON
...
func init() {
	sebaCmd.Flags().StringVarP(&sebaOutputPath, "output", "o", "", "Output HTML file path (default: ~/.config/anubis/seba.html)")
...
func runSeba(cmd *cobra.Command, args []string) {
	...
	graph := mapper.NewGraph()

	// 1. Add the workstation as the central node
	hostname, _ := os.Hostname()
	graph.AddNode("workstation", fmt.Sprintf("🖥 %s", hostname), mapper.NodeDevice)
	...
	if scanResult != nil {
		for _, f := range scanResult.Findings {
			...
			graph.AddNode(nodeID, label, mapper.NodeCache)
	...
	if err == nil {
		for _, g := range auditResult.Groups {
			...
			graph.AddNode(nodeID, label, mapper.NodeProcess)
		...
		for _, o := range auditResult.Orphans {
			...
			graph.AddNode(nodeID, label, mapper.NodeGhost)
	...
	// 4. Detect Docker
	if dockerRunning() {
		graph.AddNode("docker", "🐳 Docker Desktop", mapper.NodeService)
		graph.AddEdge("workstation", "docker", "runs")

		containers := getDockerContainers()
		for _, c := range containers {
			graph.AddNode(c.id, fmt.Sprintf("📦 %s", c.name), mapper.NodeContainer)
	...
	for _, v := range volumes {
		graph.AddNode(v.id, fmt.Sprintf("💾 %s", v.name), mapper.NodeVolume)
	...
	if ifaces := getNetworkInterfaces(); len(ifaces) > 0 {
		graph.AddNode("network", "🌐 Network", mapper.NodeNetwork)
		graph.AddEdge("workstation", "network", "connected")
		for _, iface := range ifaces {
			graph.AddNode(iface.id, fmt.Sprintf("🔗 %s", iface.name), mapper.NodeNetwork)
	...
	if outPath == "" {
		...
		outPath = filepath.Join(home, ".config", "anubis", "seba.html")
	}
